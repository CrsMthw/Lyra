...
### Added
- **Audio visualizer** — `android.media.audiofx.Visualizer(0)` captures the global output mix and drives two reactive painters: `FftCWave` (smooth Akima-spline circular filled-disc that pulses outward past the art edge, with gravity-decay physics) pulsates behind the album art in `PlayerScreen` (portrait and landscape); `FftWave` (mirrored horizontal wave) overlays the bottom scrim in `QueueScreen`; toggled via the FAB menu Visualizer button; `RECORD_AUDIO` permission requested on-demand only when the user first enables it (never at launch); off by default, persisted in DataStore; degrades gracefully if permission is denied or revoked
...
- **Media features FAB menu** — an M3 Expressive `FloatingActionButtonMenu` (`GraphicEq` icon) at the bottom-right of the art area replaces the old invisible toggle button; expands to reveal a Lyrics item and a Visualizer placeholder; lyrics item shows a `LoadingIndicator` while fetching, "No lyrics" (disabled) when unavailable, and highlights when active; FAB container uses `surfaceAccentColor` for a solid, always-visible appearance against any album art
...
- **FAB menu item colors** — menu items now use solid `surfaceContainerHigh` containers with full-opacity `outline` color for disabled/unavailable states (Visualizer placeholder, "No lyrics"); removed all `Modifier.alpha` transparency from items — the previous 12–25% alpha containers looked washed out against album art backgrounds
- **FAB position** — the `ArtFabMenu` is offset `y = 32.dp` below the art box bottom edge in both portrait and landscape, overlapping the space below the art rather than sitting inside it
